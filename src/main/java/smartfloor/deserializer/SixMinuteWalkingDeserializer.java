...
public class SixMinuteWalkingDeserializer extends StdDeserializer<SixMinuteWalking> {
    ObjectMapper mapper;

    public SixMinuteWalkingDeserializer() {
        this(null);
        mapper = new ObjectMapper();
    }

    protected SixMinuteWalkingDeserializer(Class<?> vc) {
        super(vc);
        mapper = new ObjectMapper();
    }
    ...
    @Override
    public SixMinuteWalking deserialize(JsonParser jsonParser, DeserializationContext deserializationContext)
            throws IOException {
        JsonNode smwtNode = jsonParser.getCodec().readTree(jsonParser);
        JsonNode trialsNode = smwtNode.get("trials");
        JsonNode indicatorsNode = smwtNode.get("indicators");
        ...
        for (JsonNode jsonNode : trialsNode) {
            ...
            trials.add(new SixMinuteWalking.Trial(
                    trialIndicators, beginTime, endTime
            ));
        }

        return new SixMinuteWalking(trials, indicators);
    }
}
